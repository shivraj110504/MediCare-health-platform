...
foreach ($file in $pages) {
    if (Test-Path $file) {
        ...
        # Add required stylesheets if not present
        if (-not ($content -match 'navbar-footer\.css')) {
            $content = $content -replace '(<link.*?futuristic-theme\.css".*?>)', '$1
    <link rel="stylesheet" href="styles/navbar-footer.css">'
        ...
        # Insert the navbar right after the body tag
        $content = $content -replace '<body.*?>', "`$0`n$navbarCode"
        
        # Remove any duplicate footers
        $content = $content -replace '(?s)<footer.*?</footer>', ''
        
        # Insert the footer before the closing main tag or body tag
        if ($content -match '</main>') {
            $content = $content -replace '</main>', "$footerCode`n    </main>"
        } else {
            $content = $content -replace '</body>', "$footerCode`n</body>"
        }
        
        # Remove any duplicate DOMContentLoaded event listeners
        $content = $content -replace '(?s)document\.addEventListener\(''DOMContentLoaded'',.*?\}\);', ''
        
        # Add the navbar script before the closing body tag
        $content = $content -replace '</body>', "`n    <script>$navbarScript`n    </script>`n</body>"
